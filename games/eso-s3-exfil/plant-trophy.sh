...
if aws secretsmanager describe-secret --region "${REGION}" --secret-id "${NAME}" >/dev/null 2>&1; then
    ...
    aws secretsmanager create-secret --region "${REGION}" --name "${NAME}" --secret-string "${VALUE}" \
        --tags 'Key=project,Value=watch-it-burn' 'Key=event,Value=ai-engineer-worldsfair-2026' 'Key=component,Value=exfil-game-trophy' >/dev/null
fi
