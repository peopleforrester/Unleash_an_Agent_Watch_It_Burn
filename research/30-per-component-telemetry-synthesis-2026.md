...
5. **Community/importable dashboard:** none meaningful until the SDK is wired. Background, not
   center-stage, so **no dashboard proposed** per the constraint.
...
3. **Capture in THIS stack:** **config-only**: set kagent Helm `otel.tracing.enabled: true` with
   ...
   `NO_CONTENT` on the shared path; `EVENT_ONLY` only for the re-leak beat (never `=true`, which is invalid and
...
5. **Community/importable dashboard:** n/a, background exfil target, not center-stage, so **no
