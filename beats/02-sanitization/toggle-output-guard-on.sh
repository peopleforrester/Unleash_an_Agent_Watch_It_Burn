#!/usr/bin/env bash
# ABOUTME: Beat-2 OUTPUT guard toggle — runtime flip via the guard-proxy /toggle endpoint.
...
case "${1:-}" in
    "")        STATE="on" ;;
    --off)     STATE="off" ;;
    -h|--help) echo "usage: toggle-output-guard-on.sh [--off]   (NS env overrides namespace, default 'agent')" >&2; exit 0 ;;
