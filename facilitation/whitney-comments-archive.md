<!-- ABOUTME: Durable archive of Whitney's Google Doc comments + all replies, with the section -->
<!-- ABOUTME: anchors. Backup so comments are never lost if a Doc is re-synced/overwritten. -->

# Whitney comment archive

Captured 2026-06-20 from the live Google Docs via the Drive API.

## Watch It Burn - 3 - Run of Show (18 comments)

### [3.1] Whitney Lee, 2026-06-20 (open)
- **Anchored to section:** "Cluster 1"
- **Comment:** Okay, here are the three things I think they should do in cluster one, and I'm unclear about the order:
1. They should try to exfiltrate information. This is the basketball game. This could either happen by trying to get them to exfiltrate Kubernetes secrets, but maybe it'll be more interesting to have an app in the cluster that's streaming fake customer data to another app in the cluster. Their job is to get some of that customer data into the S3 bucket.

2. Another thing we could have them do is maybe they each have their own malicious app, we'll give them an image and an already built thing. Maybe the part that makes it theirs is that it serves a static image of a certain villain. Someone gets a Joker malicious app, someone else has Mr. Burns from The Simpsons malicious app, and someone else has, I don't know, Gargamel. They can keep track of what's theirs, and they try to get the agent to deploy their malicious app onto the cluster.

3. The third attack path is to kill the cluster. We can just teach them what a fork bomb is, give them a fork bomb command, and the first person who gets the agent to run it will kill the cluster.
Those are the three attacks I'm thinking of.



When it comes to cluster two, now have them try and run the same three attacks:
1. The one where the customer data is exfiltrated is blocked by two things, actually:
- Network policy should be able to prevent the agent from sending data outside the cluster, although I'm unclear about that one because it is talking with a human outside the cluster, for example.

- The encryption provided by Istio, the pod-to-pod encryption, makes it so even if it's exfiltrated, it's unreadable. I think Cillium might be able to do L7 pod-to-pod communication too, as well as network policy. Cilium also uses Spiffe for identity under the hood, I believe.
  - **Reply, Whitney Lee (2026-06-20):** 2. As far as running a malicious app, that's where we show Kyverno putting a stop to that happening. It won't pull from an untrusted registry, or the images have to be signed if we want to demo the Harbor piece. We should look into that and how those two technologies play together, exactly. But worst case, we leave the signing piece out of it and show that Kyverno blocks any image pulls that aren't from our internal harbor registry.

3. The third thing is people trying to run executable code, specifically a fork bomb, and we can show that being blocked by Falco
...
- **Comment:** What;s this?
  - **Reply, Michael Forrester (2026-06-20):** [Build update] Full mechanism + roles map (how this is actually wired, where each guardrail lives, what invokes what) is in the new doc 'Watch It Burn - 7 - Stack Walkthrough': https://docs.google.com/document/d/1l5JqnSqTbFgo-J0cXFd_jUdM21HA7PO4n1ttl42nR-c/edit
  - **Reply, Michael Forrester (2026-06-20):** [Build update] The trace re-leak trap: output sanitization scrubs the FAKE sentinel from the agent's REPLY, but if OTel content-capture is on, the same sentinel lands in the trace SPAN (and flows to Datadog). Observability becomes a second, unguarded exfil sink. The fix is symmetric: a redaction processor in the OTel Collector scrubs it before export. It is now a Cluster-3 game ('trace re-leak hunt') and is explained in Doc 7 (Stack Walkthrough).
...
Is that what you were thinking?
  ...
  - **Reply, Michael Forrester (2026-06-20):** [Build update] Full mechanism + roles map (how this is actually wired, where each guardrail lives, what invokes what) is in the new doc 'Watch It Burn - 7 - Stack Walkthrough': https://docs.google.com/document/d/1l5JqnSqTbFgo-J0cXFd_jUdM21HA7PO4n1ttl42nR-c/edit
  - **Reply, Michael Forrester (2026-06-20):** [Build update] Decision: staying on VPC-CNI + Istio ambient, NOT switching to Cilium. Istio already gives mTLS + SPIFFE workload identity (its certs ARE SPIFFE SVIDs). Cilium would only add L7/DNS-aware egress, which the demo does not need, and it is a CNI swap with no payoff here. If a specific L7-egress need shows up we will revisit. Harbor + cosign image signing is being added.

### [3.18] Whitney Lee, 2026-06-19 (open)
