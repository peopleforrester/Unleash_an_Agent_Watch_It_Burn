...
Spec rev3, 2026-06-16. Supersedes rev2. This file is the single source of truth for Claude Code.

> **rev3 ARCHITECTURE CHANGE (authoritative — overrides any "vCluster" wording still in this file pending full reconciliation):** Per-attendee isolation is now a **separate EKS cluster per attendee**, in a **hub-and-spoke** topology. A small **hub** EKS cluster runs ArgoCD plus the facilitator's Grafana/Tempo. Each attendee gets their own **spoke** EKS cluster, registered to the hub ArgoCD, which delivers the full IDP stack into every spoke via an ApplicationSet **cluster generator**. vCluster is removed. LLM Guard runs **per-spoke, output-`Regex`-only by default** (Sensitive NER is opt-in) to keep each spoke node small. Cost and AWS service quotas scale linearly with N — see §10/§11. If the live abstract and this file disagree, the abstract's *Proposed Amended Description* wins on behavior, and this file is updated to match.
...
- **Per-attendee isolation via a separate EKS cluster (hub-and-spoke).** One managed EKS spoke cluster per attendee; a small hub cluster runs ArgoCD + Grafana/Tempo. No vCluster, no namespace-only multi-tenancy. Rationale: physical isolation, and this workshop installs per-attendee admission webhooks (Kyverno), runtime security (Falco needs real node/kernel access), and multiple operators/CRDs (kagent, ArgoCD-managed content, agentgateway) — all cleaner on a real cluster than virtualized. Cost and provisioning time scale with N (§10/§11).
