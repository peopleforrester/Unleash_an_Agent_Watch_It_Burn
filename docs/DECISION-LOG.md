...
| 2026-06-27 | PRD #28 platform UST can be acceptance-tested live via `GET /api/v1/services` | `/api/v1/services` is not a documented public Datadog endpoint, and the documented `GET /api/v2/services/definitions` only lists services with a registered `service.datadog.yaml` — UST-telemetry-discovered components (no definition file) never appear there. No stable public API confirms "telemetry service X present with `env:production`"; that is a Software Catalog UI check | Verified against the live Datadog API docs (service-definition + unified-service-tagging pages, 2026-06-27) before writing any live assertion | An acceptance method named in a PRD can still be a non-existent API; verify the endpoint against live docs before coding a test, and make the deterministic gate the static manifest check (`verify/test_observability.py`), not a fabricated live call |
