...
## 2026-09-06 · Every hand fix of the week wired into the process (#251 to #257)

The owner's question: which fixes were applied to state and left out of the process. The answer was six,
each now a verb with a test: ai-layer ConfigMaps and Secrets carry the kustomize hash so content changes
roll their pods (#252, superseding the 2026-09-03 rejection); the bootstrap roots manage themselves as
multi-source Applications (#253); converge repairs identity, keys, dual shipping, stuck operations, stuck
pods and empty states and runs the Datadog verifier, and up ends with that pass (#254); a fleet teardown
ends with a zero audit (#255); the Datadog pool loads through a validating script (#256); the compiled
design Doc is generated by a script that owns the Doc id (#257).

Two things only that work exposed. The ai-layer env ignore hid the hashed Secret reference from Argo CD,
so guard-proxy pointed at a pruned Secret on every cluster until the ignore was narrowed to literal
entries. And the customer-stream Deployments had never deployed on an attendee cluster: require-labels and
