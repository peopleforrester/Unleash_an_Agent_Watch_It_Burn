...
**Priority**: High
**Status**: M1 (mechanism) implemented 2026-06-25: AWS Secrets Manager + ESO fan-out to all three namespaces, populated per-attendee by `lab-distribution/scripts/distribute_datadog_keys.py`. M2 (Whitney's live verification) pending.
...
- [x] The secret contents match that attendee's Datadog trial org credentials from `pool.csv` (the loader writes each row's exact keys)
- [x] **CRITICAL: credentials are never printed to the terminal, committed to git, or logged** (loader never prints values; nothing committed)
