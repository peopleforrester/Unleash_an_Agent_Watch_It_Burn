...
**Priority**: Medium
**Status**: Implemented (2026-06-27). UST pod annotations added to all five platform components, gated by a deterministic static check in `verify/test_observability.py` (green) and a live check in `verify/test_datadog_service_map.py` (`test_platform_component_ust`, runs against the Datadog API). On `main` and cherry-picked to all four whitney branches. The only remaining manual step is eyeballing the Service Catalog / Service Map UI rendering, which is a convenience look, not a blocker (see Decision Log 2026-06-27).
...
- [x] All five platform components (ArgoCD, Kyverno, Falco, cert-manager, Istio ambient) carry `tags.datadoghq.com/{service,env,version}` with `env=production` (static gate: `verify/test_observability.py`, green)
- [x] Each component reports live metrics tagged `service:<name>` + `env:production` (live gate: `verify/test_datadog_service_map.py::test_platform_component_ust`, via `GET /api/v2/metrics` + `GET /api/v1/query`; superseded the non-existent `GET /api/v1/services` method per Decision Log 2026-06-27)
- [x] No AI-layer component UST values changed
...
| 2026-06-27 | The PRD's `GET /api/v1/services` acceptance method does not exist; replaced with a metrics-tag live gate. | Verified against the live Datadog API docs (2026-06-27): `GET /api/v1/services` is not a documented public endpoint, and the documented `GET /api/v2/services/definitions` lists only services that have a registered `service.datadog.yaml` definition — our components are discovered purely from UST telemetry tags (no definition file) and so never appear there. The catalog "list services" view has no stable public API. The verifiable, equivalent fact IS automatable: that each component's live metrics carry `service:<name>` + `env:production`. `verify/test_datadog_service_map.py::test_platform_component_ust` does this name-agnostically — `GET /api/v2/metrics?filter[tags]=service:<name>` discovers a reporting metric (no hard-coded integration metric name), then `GET /api/v1/query` confirms live points scoped to `service:<name>` + `env:production`. The static manifest gate (`verify/test_observability.py`) remains the offline check. Only the Service Catalog/Map UI rendering needs a manual look. Sources: https://docs.datadoghq.com/api/latest/metrics/get-a-list-of-metrics/ , https://docs.datadoghq.com/api/latest/service-definition/ , https://docs.datadoghq.com/getting_started/tagging/unified_service_tagging/. |
| 2026-06-27 | The Datadog app key IS deployed in-cluster (corrects an earlier claim). | `gitops/manifests/datadog/datadog-eso.yaml` syncs both `api-key` and `app-key` from the `watch-it-burn/datadog` Secrets Manager secret into the in-cluster `datadog-secret`; `gitops/manifests/datadog/datadog-agent.yaml` consumes the app key via `global.credentials.appSecret` (the Datadog Cluster Agent uses it). So the live gate above is not blocked by a missing key. An earlier comment (and PRD #33's acceptance note) said the app key was "not deployed in the cluster"; that was inaccurate and was corrected on the issues. |
