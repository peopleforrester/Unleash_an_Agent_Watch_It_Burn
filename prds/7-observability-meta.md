...
5. ~~**Service Map from pure OTLP (live-verify)**~~ — **Resolved (2026-06-25 Decision Log).** Acceptance: `GET /api/v1/service_dependencies` must return the expected edges (`guard-proxy→LLM Guard`, `guard-proxy→agentgateway`, `agentgateway→kagent`, `kagent→Bedrock`); if Bedrock is external-only, 3 internal edges is acceptable — verify-at-build which edges the live cluster actually emits. Child PRD includes a debug path (check span attributes via `GET /api/v1/traces` or Datadog MCP; confirm `peer.service` and span kind are set correctly on CLIENT spans; re-run map query). No fallback to Agent trace path — if OTLP Service Map fails, fix the OTLP pipeline.
...
2. Run `/prd-create` for a child PRD per decisions 1-5. **Scope: AI layer only** (platform component UST is a separate future PRD — 2026-06-25 Decision Log):
   ...
   - **Milestone working pattern**: each implementation bullet follows an iterate loop — implement → deploy to cluster → check Datadog (query `GET /api/v1/service_dependencies`, query Datadog logs API, or check UST in Datadog UI) → diagnose if wrong → adjust → re-check. The acceptance script is the final gate, not the first check. Michael should expect multiple deploy-check-adjust cycles per bullet before the gate passes.
   - Run `/prd-update-decisions` inside the child PRD session.
3. Run `/prd-create` for the platform component UST PRD (created now as a backlog issue; implemented after the AI layer child PRD is merged — 2026-06-25 Decision Log entry 603):
   - Scope: add `tags.datadoghq.com/*` pod annotations to third-party Helm charts: ArgoCD, Kyverno, Falco, cert-manager, Istio ambient, and any other platform components identified in `research/30-per-component-telemetry-synthesis-2026.md`
   - **Milestone working pattern**: implement pod annotations → deploy → check Datadog (query `GET /api/v1/service_dependencies`, check Service Map in Datadog UI) → diagnose if component or edge is missing → adjust annotations or restart collector → re-check. Each milestone is not done until the check passes; expect multiple iterations per component.
   - One milestone per component: implement pod annotations → run `GET /api/v1/service_dependencies` → assert component appears with expected edges → iterate until passing
   - Acceptance per component: extend `verify/test_datadog_service_map.py` (or a companion script) to assert each component's Service Map presence and edges
   - This PRD is a backlog item only at M6 creation time — it is NOT part of M6's acceptance criteria
4. Add both new PRDs to `docs/ROADMAP.md`: AI layer child PRD as `- UST, Service Map & correlation (PRD #[issue-id])` after Milestone 5; platform component UST PRD as `- Platform component unified service tagging (PRD #[issue-id])` after that entry.
5. Run `/prd-update-progress` to commit + push.
6. Instruct the user to start a new session, then run `/prd-next` for Milestone 7.
...
- **Prerequisite gate (Updated per 2026-06-24 Grafana decision):** Issue [#24](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/24) (Datadog community dashboard research spike) must be complete and its file path posted as a comment. ✅ Complete — `research/35-datadog-community-dashboards-2026.md`.
...
**Step 2 — Resolve with Whitney (one at a time):**
1. ~~**Import which Datadog community dashboards?**~~ **Pre-resolved (2026-06-25 Decision Log).** Rule: import only if zero-work AND data confirmed flowing — an empty dashboard signals "something's broken." Per-component decisions: **(a) cert-manager** — import the OOTB "Cert Manager Overview" dashboard; `cert_manager.*` metrics confirmed flowing. **(b) Istio** — skip; OOTB `istio_overview.json` is L7 sidecar-only and renders mostly empty for our L4-only ztunnel deployment; Istio OOTB dashboard import was added as an acceptance criterion to [issue #25](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/25) (waypoint proxy) — import happens there when L7 metrics flow; no custom ztunnel dashboard (Decision 5 2026-06-25). **(c) Kyverno** — import OOTB dashboard directly; no OTTL rename needed (Agent openmetrics check emits `kyverno.*` dot-format names natively; confirmed flowing on live cluster — see Decision Log D4 2026-06-25). Issue #31 closing. **(d) ESO** — skip; no Datadog integration, not worth the wiring effort. These decisions are locked — do not re-open without new information.
2. ~~**Custom/story dashboards** — for each candidate (Wasted Tokens Over Time, Model Tier Cost Race [group by `gen_ai.request.model`], Tool Call Heatmap, Guardrail Toggle Timeline): **build now, defer to dress rehearsal, or skip?** Confirm the data source is flowing before committing to build.~~ **Pre-resolved (2026-06-25 Decision Log D2).** All four deferred to dress rehearsal — LLM agent telemetry needed to validate these dashboards is not present until dress rehearsal.
3. ~~**Dashboard JSON as code (committed)** vs. UI-built?~~ **Pre-resolved (2026-06-25 Decision Log D3).** Terraform `datadog_dashboard_json` resources in `infra/terraform/dashboards/`. OOTB dashboards that auto-install via Agent checks do not need Terraform resources.
...
- [x] Decision 4 (app keys in cluster secret?) resolved with Whitney
...
| 2026-06-24 | ~~M5 Decision 9: Kyverno native OTLP (`otelConfig=grpc`) — enable; Agent Prometheus check — skip~~ **(SUPERSEDED 2026-06-25 — see D4: gRPC-Go dns resolver bug in Kyverno binary; OTLP produces zero telemetry; Agent openmetrics check is the only viable path)** | ~~Enable Kyverno's native OTLP by setting `otelConfig: grpc` and `otelCollector: <in-cluster-otel-collector-endpoint>` in the Kyverno Helm values (`gitops/apps/kyverno.yaml` `helm.valuesObject`). This sends both Prometheus-format **metrics** AND policy-decision **traces** to the Collector (→ Datadog APM + metrics backend). The Agent `kyverno` Prometheus check is skipped — enabling both would duplicate metrics and add billing noise. Rationale: Kyverno is center-stage (Audit→Enforce toggle, Beat 1); native OTLP aligns with the Datadog-additive OTel principle and gives traces that the Agent check cannot provide. The Collector → Datadog routing is already established; this is an additive sender-side config only.~~ *Both OTLP tracing and metering blocks removed from `gitops/apps/kyverno.yaml` (commit `8ed0a0c`, 2026-06-25). Switched to Agent Autodiscovery openmetrics check. kyverno.* metrics confirmed flowing.* |
...
| 2026-06-25 | M6 produces two child PRDs via /prd-create: AI layer PRD and platform component UST PRD | (1) AI layer child PRD: agentgateway UST fix, guard-proxy CLIENT span + `peer.service`, OTTL fallback, log-trace correlation, Weaver CLIENT span group, Service Map verify — created first and implemented immediately by Michael. (2) Platform component UST PRD: add `tags.datadoghq.com/*` pod annotations to third-party Helm charts (ArgoCD, Kyverno, Falco, cert-manager, Istio ambient, and any other platform components identified in `research/30-per-component-telemetry-synthesis-2026.md`); milestone-per-component + Datadog API verification loop (see Decision Log entry 591); created now as a GitHub issue to give it a backlog home but implemented in a future sprint after the AI layer PRD is merged. Both `/prd-create` calls happen during the M6 design session. |
| 2026-06-25 | M7 dashboard import rule: import only if zero-work AND data confirmed flowing | The general rule for OOTB and community dashboard imports in M7: import if and only if it is zero-work (Datadog auto-installs on Agent check activation or one-click JSON import) AND the underlying data is confirmed flowing. An empty dashboard signals "something's broken" rather than "Datadog sees everything" — worse than no dashboard. This rule governs all M7 decisions. |
| 2026-06-25 | M7 Decision 1a: cert-manager OOTB dashboard — import | The "Cert Manager Overview" OOTB dashboard ships with the Datadog Agent `cert_manager` check. `research/35` confirmed `cert_manager.certificate.expiration_timestamp`, `.ready_status`, and `.controller.sync_call.count` are flowing live on `watch-it-burn-attendee-001`. Zero-work import — Datadog auto-installs the dashboard when the Agent check is active. |
| 2026-06-25 | M7 Decision 1b: Istio OOTB dashboard — skip; import gated on issue #25 (waypoint proxy) | The OOTB `istio_overview.json` is built for L7 sidecar/Envoy metrics (`istio.mesh.*`). Our ztunnel-only ambient deployment emits L4 metrics only; the OOTB dashboard renders mostly empty. An empty dashboard is worse than none — it signals a broken integration. The Istio dashboard import is NOT deferred as a note in the M7 child PRD; instead it was added as an acceptance criterion directly to [issue #25](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/25) (Istio ambient waypoint proxy). When L7 Envoy metrics flow via the waypoint, the dashboard will populate and can be imported as part of that work. If issue #25 is never done, the Istio OOTB dashboard is never imported. |
| 2026-06-25 | ~~M7 Decision 1c: Kyverno dashboard — import OOTB via OTTL metric rename in Collector; gated on issue #31~~ **(SUPERSEDED 2026-06-25 — see D4 below: gRPC-Go dns resolver bug in Kyverno binary; OTLP path abandoned; Agent check confirmed; OTTL rename not needed; OOTB dashboard importable directly)** | ~~Kyverno's native OTLP SDK emits underscore metric names (e.g., `kyverno_policy_results`). The OOTB Kyverno dashboard expects dot-format Agent check names (e.g., `kyverno.policy.results.count`). The Agent Kyverno check is skipped per M5 D9 (OTLP to avoid duplicates). Approach: add an OTTL transform processor in `gitops/apps/otel-collector.yaml` to rename Kyverno OTLP metric names from underscore to dot-format before Datadog export. This keeps OTLP (no duplicates, vendor-agnostic for attendees using other backends) and feeds the OOTB dashboard. **This approach is gated on issue #31** (Kyverno OTLP NetworkPolicy bug: zero `kyverno*` metrics reach Datadog currently). Michael must fix the NetworkPolicy, confirm metrics arrive in Datadog, and post the observed metric names as a comment on issue #31 before the M7 child PRD can include this work. The M7 child PRD reads issue #31's comment as a prerequisite gate. Rejected alternatives: (1) switch Kyverno to Agent check — rejected because M5 D9 chose OTLP to avoid duplicates; (2) build a custom dashboard against underscore names — rejected because the OTTL rename approach gets the OOTB dashboard for free with no custom build.~~ |
| 2026-06-25 | M7 Decision 1d: ESO dashboard — skip | No official Datadog integration for ESO exists. Building a custom dashboard would require wiring an Agent OpenMetrics scrape of ESO first, then building from `externalsecret_*` metrics. ESO is a background infrastructure component with no workshop narrative role. Not worth the effort. |
| 2026-06-25 | M7 issue #25 update: Istio OOTB dashboard import added as acceptance criterion to issue #25 | "Import the Datadog OOTB Istio dashboard (`istio_overview.json`) — L7 Envoy metrics from the waypoint will populate it; confirm dashboard renders with data before marking complete" was added to issue #25's acceptance criteria on 2026-06-25. The M7 child PRD does not reference this — it belongs entirely to the optional issue #25 scope. |
| 2026-06-25 | ~~M7 Kyverno OTLP bug filed as issue #31~~ (SUPERSEDED 2026-06-25 — root cause confirmed as gRPC-Go binary bug, not NetworkPolicy; see D4 below; issue #31 closing) | ~~A live defect (NetworkPolicy blocking Kyverno admission controller egress to OTel Collector on port 4317) was discovered in `research/35` and filed as [issue #31](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/31). This is post-PRD-#26 (closed). The M7 child PRD lists issue #31 as a prerequisite gate: Michael must post the observed metric names (exact underscore format) as a comment on #31 before M7 adds the OTTL rename and imports the OOTB Kyverno dashboard.~~ *Root cause confirmed by Michael (2026-06-25): not NetworkPolicy (no NetworkPolicy in kyverno namespace; TCP 4317 open; DNS resolves). Actual cause: gRPC-Go `grpc.NewClient()` dns resolver in the Kyverno binary. Issue #31 closing.* |
| 2026-06-25 | M7 Decision 4: Kyverno observability — Agent openmetrics check only; OTLP path abandoned (SUPERSEDES M5 D9 and D1c) | Root cause of OTLP failure confirmed by Michael (2026-06-25): gRPC-Go `grpc.NewClient()` (introduced v1.63) defaults to `dns` resolver instead of `passthrough`. Even a bare ClusterIP fails with `name resolver error: produced zero addresses` because the dns resolver queries DNS for a numeric IP literal and returns nothing. This is NOT a NetworkPolicy issue (no NetworkPolicy in kyverno namespace; TCP 4317 open; DNS resolves) — it is a behavior change baked into the Kyverno binary's bundled gRPC/OTel exporter that cannot be fixed without a Kyverno fork. **Implemented in commit `8ed0a0c`:** both the `tracing:` block (OTLP APM traces) AND the `metering.config: grpc` block (OTLP metrics) are removed from `gitops/apps/kyverno.yaml`. Switched to Datadog Agent Autodiscovery openmetrics check (`ad.datadoghq.com/kyverno.checks` pod annotation on the admissionController) — same pattern as cert-manager, Falco, and Istio ambient. `kyverno.*` metric names confirmed flowing on the live cluster; they match the OOTB Kyverno dashboard's expected names. Issue #31 closing. `research/35` Kyverno row updated to reflect resolution. **Consequences for Beat 1:** Kyverno APM traces (policy-decision spans) are not achievable with the current Kyverno binary — Beat 1 observability is metrics-based only (OOTB Kyverno dashboard). The OTTL metric rename from D1c is not needed — Agent check emits `kyverno.*` dot-format names natively. OOTB Kyverno dashboard imports directly with no additional configuration work. |
| 2026-06-25 | M7 Decision 5: No custom Istio/ztunnel dashboard | Michael proposed building a custom dashboard against ztunnel L4 TCP metrics (ambient L4 stats). Whitney rejected: no custom Istio dashboard of any kind in this project. The existing D1b decision (Istio OOTB dashboard import gated on issue #25, added to #25 acceptance criteria) stands unchanged. M7 scope includes no Istio dashboard work — neither OOTB import nor custom. Istio dashboard work lives entirely in optional issue #25; if that issue is never done, no Istio dashboard is ever imported. |
| 2026-06-25 | M7 Decision 6: ArgoCD Autodiscovery annotation confirmed [OK]; issue #32 is moot | The Datadog Agent `argocd` check is already active. `infra/argocd-values.yaml` (the correct file for bootstrap-installed ArgoCD — the ArgoCD instance itself, not gitops-managed apps) already has `ad.datadoghq.com/server.checks` with `api_server_endpoint: "http://%%host%%:8083/metrics"` since commit `3866970` (Michael changed `openmetrics_endpoint` → `api_server_endpoint` in that commit). Issue #32 was filed targeting `gitops/argocd/values.yaml` — the wrong file; that file is the gitops Application definition, not the ArgoCD server configuration. Issue #32 should be closed as moot (the annotation was never missing from the correct file). No action required in M7 for ArgoCD — OOTB ArgoCD dashboard imports directly. |
| 2026-06-25 | M7 Decision 2: Custom/story dashboards — all four deferred to dress rehearsal | Four story-specific dashboard candidates: Wasted Tokens Over Time, Model Tier Cost Race (group by `gen_ai.request.model`), Tool Call Heatmap, Guardrail Toggle Timeline. Decision: all four deferred to dress rehearsal. Rationale: none of these can be meaningfully built now because the LLM agent telemetry that drives them (real `gen_ai.*` spans from a live workshop run; actual per-model cost data from production-like load) is not present in the stack until dress rehearsal. Building skeleton dashboards against absent data produces empty charts that cannot be validated. Building at dress rehearsal — when the data is live — allows dashboard design to be validated against real signals before the workshop. The M7 child PRD does not include any custom dashboard build work. |
| 2026-06-25 | M7 Decision 3: Dashboard JSON as code via Terraform `datadog_dashboard_json` resources in `infra/terraform/dashboards/` | Dashboard definitions are committed to the repo as Terraform `datadog_dashboard_json` resources in a new `infra/terraform/dashboards/` module. OOTB dashboards that auto-install when the Datadog Agent check is active (cert-manager, Kyverno, ArgoCD) do not need Terraform resources — Datadog manages them automatically. The Terraform module is scaffolded in M7 as the foundation; custom dashboards (deferred to dress rehearsal per Decision 2) will be added to this module when built. The Terraform approach follows the existing `infra/terraform/` directory convention in this repo and enables GitOps-style PR review for dashboard changes. |
