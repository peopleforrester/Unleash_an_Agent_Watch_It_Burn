...
2. Run `/prd-create` for a child PRD per decisions 1-5. **Scope: AI layer only** (agentgateway UST fix, `peer.service`, log-trace correlation, Service Map verify — platform component UST is a separate future PRD per 2026-06-25 Decision Log). Acceptance: `verify/test_datadog_service_map.py` passes (4 expected edges: `guard-proxy→agentgateway→kagent→Bedrock`, verified via `GET /api/v1/service_dependencies`) **and** both log-trace pivot directions pass (forward: logs query by `trace_id` returns results; reverse: APM trace shows associated logs). **No "renders in the UI" browser assertion** — all acceptance is Datadog API-based (2026-06-25 Decision Log). Run `/prd-update-decisions` inside the child PRD session.
...
- [ ] A child PRD issue exists (AI layer only) scoped to: agentgateway UST fix, `peer.service`, log-trace correlation, Service Map verify
- [ ] Child PRD acceptance criteria: `verify/test_datadog_service_map.py` passes (4 edges: `guard-proxy→agentgateway→kagent→Bedrock` via `GET /api/v1/service_dependencies`) AND both log-trace pivot directions pass (query logs by `trace_id` → results; query APM trace → associated logs present)
- [ ] ROADMAP updated
