...
- **Already wired:** `spanmetricsconnector` with `add_resource_attributes: true`; UST tags via `OTEL_RESOURCE_ATTRIBUTES` on guard-proxy/agentgateway/kagent; `cluster.name=watch-it-burn` upserted by the Collector `resource` processor.
...
3. **Wire-or-skip per named integration** — one component at a time.
...
4. **Log-trace correlation for Python apps** — does UST alone suffice, or do Python stdout logs need explicit `trace_id`/`span_id` fields? (`research/19`: stdout/file pipeline needs explicit fields.) If injection is needed: Python OTel SDK log bridge vs. manual extraction from active span context. Run a `/research` or live check.
