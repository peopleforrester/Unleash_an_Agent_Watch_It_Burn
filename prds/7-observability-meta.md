...
- **Issue #18** — evil-mcp-shim work item: reconcile drifted `server.py` copies (add `apply_optimization` to `gitops/ai-layer/server.py` using hardcoded fallback) and add UST env vars to `gitops/ai-layer/resources.yaml`. **No OTel instrumentation** (decided 2026-06-24 — shim is visible as the agent's `execute_tool` spans; see issue #18 Decision Log). The M2 child PRD should include this issue as an implementation work item.
