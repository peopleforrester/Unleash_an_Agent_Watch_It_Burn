...
**Step 2 — Resolve with Whitney (one at a time):**
1. **Read the pre-completed research spikes before proceeding** — issues #9, #10, and #15 are prerequisites that must be complete before this design conversation. Get the research file path from the comment on each issue, then read all three files:
...
2. **OTel SDK delivery mechanism (issue #15 prerequisite)** — Confirm issue #15 research is complete and its file path is posted as a comment. Then resolve: whether the OTel Operator is deployed alongside the standalone contrib 0.158.2 Collector, or per-component ad-hoc delivery is used for the custom Python pods (evil-mcp-shim in M2, guard-proxy in M3). Also decide: how `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`, and `OTEL_RESOURCE_ATTRIBUTES` are configured cluster-wide (Instrumentation CRD, Helm values, pod env, or mix). This decision is a prerequisite for Decision 3 and is inherited by M3 for guard-proxy — record it as a compound decision before proceeding.
3. **Instrumentation approach per Python component** — read issue #10's research output and decide, for each of the **three** components with LLM calls (kagent/ADK, agentgateway, evil-mcp-shim), which approach produces OTel semconv-compliant spans. Do NOT select OpenInference — Datadog does not support it. The target standard is OTel semantic conventions throughout, specifically OTel GenAI semantic conventions (`gen_ai.*` attributes and span names) for AI operations. Options: native built-in OTel, an auto-instrumentation library (OpenLLMetry has historically provided OTel GenAI semconv auto-instrumentation for Python and is Datadog-supported at 0.47+, but verify its current status — it is being absorbed into the OpenTelemetry project itself), or manual Python OTel SDK spans. Note: kagent/ADK may need nothing (ADK's OTel output is native). **guard-proxy is excluded from this decision** — it proxies requests but does not make LLM calls; it has no gen_ai.* to emit, and adding OTel spans to proxy.py is M3 scope. Issue #10 must be complete before this decision is made. Record the per-component decisions in the child PRD's Decision Log.
...
**Done when:**
- [ ] Issues #9, #10, and #15 research files confirmed complete (file paths posted as comments on each issue)
...
method chosen from issue #14 research and declared as an ArgoCD Application in GitOps. EKS
...
**Step 2 — Resolve with Whitney (one at a time):**
1. **Agent install method and configuration (issue #14 prerequisite)** — Confirm issue #14 research is complete and its file path is posted as a comment. Then resolve: (a) install mechanism — Helm chart managing Agent DaemonSet directly, Datadog Operator via Helm, EKS add-on, or other — with rationale for ArgoCD GitOps + EKS + ~60–70 cluster scale; (b) required feature flags — which capabilities to enable (log collection, container monitoring, process collection, etc.) and which to leave off, cross-referenced against `research/24` §2.3 sizing; (c) IAM: IRSA vs EKS Pod Identity vs not required; (d) Cluster Agent: required or optional; (e) GitOps manifest shape: what the Agent ArgoCD Application looks like and where it lives in `gitops/`. Record all five as a single compound decision before proceeding.
