...
- **Prerequisite gate (Updated per 2026-06-25 M7 design decisions):** Issue [#31](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/31) (Kyverno OTLP NetworkPolicy fix) must be resolved and observed metric names posted as a comment before the Kyverno OTTL rename + OOTB dashboard work proceeds. cert-manager, Istio, and ESO dashboard decisions are pre-resolved (see Decision Log 2026-06-25) and do not require #31.
...
| 2026-06-25 | M7 Kyverno OTLP bug filed as issue #31 | A live defect (NetworkPolicy blocking Kyverno admission controller egress to OTel Collector on port 4317) was discovered in `research/35` and filed as [issue #31](https://github.com/peopleforrester/Unleash_an_Agent_Watch_It_Burn/issues/31). This is post-PRD-#26 (closed). The M7 child PRD lists issue #31 as a prerequisite gate: Michael must post the observed metric names (exact underscore format) as a comment on #31 before M7 adds the OTTL rename and imports the OOTB Kyverno dashboard. |
