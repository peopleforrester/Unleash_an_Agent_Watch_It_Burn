...
| 2026-06-25 | M6 Decision 5 resolved: Service Map from pure OTLP — binary acceptance gate with debug path | Acceptance criterion is binary: `GET /api/v1/service_dependencies` must return all 4 expected edges (`guard-proxy→agentgateway→kagent→Bedrock` chain plus guard-proxy→LLM Guard CLIENT span edge) or M6 is not done — no partial pass. Child PRD includes a debug path for Michael: if edges are missing, check span attributes on relevant spans (via `GET /api/v1/traces` or Datadog MCP), confirm `peer.service` and span kind are set correctly on CLIENT spans, then re-run the map query. No fallback to Agent trace path — if OTLP Service Map fails, fix the OTLP pipeline. |
