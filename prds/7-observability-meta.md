...
4. **Log-trace correlation for Python apps** — **Context (read `agent/gateway/guard-proxy/proxy.py` to confirm):** guard-proxy currently emits zero log output — `log_message` returns `None` (silences the BaseHTTPRequestHandler access log) and there are no `print()` or `logging` calls anywhere in the file. This means the correlation question is NOT "which format are the existing logs in" but "need to add new structured JSON logging from scratch." Then decide: stdlib `logging` module with a JSON formatter vs. other approach; how to extract `trace_id`/`span_id` from the active OTel span context (`opentelemetry-api` is available via the Operator injection after M3); which pipeline the new logs land on (OTLP log bridge vs. file/stdout). On file/stdout the `trace_id`/`span_id` fields must be explicit in the log JSON (`research/19`); the Datadog Agent auto-recognizes OTel-standard field names. This likely needs a live-cluster `/research` check — run it before presenting the decision.
...
2. Run `/prd-create` for a child PRD per decisions 1-5. **Scope: AI layer only** (platform component UST is a separate future PRD — 2026-06-25 Decision Log):
   ...
   - Verify `weaver/registry/` exists; if not, create it with `registry_manifest.yaml` declaring the OTel semconv `dependencies:` entry (2026-06-25 Decision Log — bootstrap step; see Weaver scope entry for what does/does not need a local group)
   - Add guard-proxy CLIENT span group to `weaver/registry/`; run `weaver registry check` locally (2026-06-25 Decision Log)
   - Resolve log-trace correlation for Python apps (Decision 4) and Service Map live-verify (Decision 5) per this design conversation
   - Acceptance: `verify/test_datadog_service_map.py` passes (4 edges: `guard-proxy→agentgateway→kagent→Bedrock` via `GET /api/v1/service_dependencies`) **and** both log-trace pivot directions pass **and** `weaver live-check` run as terminal step. **No "renders in the UI" browser assertion** — all acceptance is Datadog API-based (2026-06-25 Decision Log).
...
- [ ] Child PRD acceptance criteria: `verify/test_datadog_service_map.py` passes (4 edges: `guard-proxy→agentgateway→kagent→Bedrock` via `GET /api/v1/service_dependencies`) AND both log-trace pivot directions pass AND `weaver registry check` passes AND `weaver live-check` run as terminal acceptance step
- [ ] ROADMAP updated
...
| 2026-06-25 | M6 child PRD must defensively bootstrap `weaver/registry/` — not guaranteed to exist when Michael starts | `weaver/registry/` does not currently exist in the repo. PRD #20 M6 plans to create it, but M6's child PRD cannot rely on that being done first (PRD #20 M6 has not yet been implemented). M6's child PRD must include step 1: check whether `weaver/registry/` exists; if not, create it with a `registry_manifest.yaml` declaring the OTel semconv community registry as a `dependencies:` entry. This is an explicit correction to the prior "can assume all upstream work is done" entry — that principle holds for code and manifest work, but `weaver/registry/` is a docs/schema deliverable whose existence cannot be assumed. |
