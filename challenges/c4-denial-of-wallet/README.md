...
live **cost counter** on BurritoBot climbs and, on the Community cluster (the burn profile, no guardrails),
nothing stops it. The damage is the invoice.
...
| Where | Outcome | Why |
|---|---|---|
| Community cluster (burn profile, no guardrails) | bill runs away | `budget` guard off; every request reaches Bedrock, counter climbs unbounded |
...
- A **runtime toggle** (`GUARDS["budget"]`, flipped via `/toggle?budget=on` or the `guard-budget-on`
  terminal script), so it is turned on live rather than through a pod-restarting env change that would
